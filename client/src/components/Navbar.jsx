/**
 * Midnight Circuit header: compact glass navigation with a clear route hierarchy and responsive mobile drawer.
 * It preserves the public links and ticket CTA behavior used throughout the application.
 */
  ...
  return <>
      ...
      <div className="container ft-header-shell">
        <nav className="ft-header-bar" aria-label="Primary navigation">
          ...
          <div className="ft-desktop-nav">
            ...
            <Link to="/events" className="ft-ticket-cta"><i />Get tickets</Link>
          </div>
    ...
    <div className={`ft-mobile-drawer ${menuOpen ? 'is-open' : ''}`} aria-hidden={!menuOpen}>
      <div className="ft-mobile-drawer-inner">
        ...
        <Link to="/events" className="ft-mobile-ticket"><i />Browse live tickets</Link>
      </div>
    </div>
...
const headerCss = `
  ...
  .ft-ticket-cta,.ft-mobile-ticket { display:inline-flex; align-items:center; justify-content:center; gap:8px; border:1px solid var(--gold); border-radius:11px; padding:10px 13px; background:var(--gold); color:#03141a; font-size:11px; font-weight:900; letter-spacing:.04em; text-decoration:none; transition:transform .16s ease,box-shadow .16s ease,background .16s ease; }.ft-ticket-cta:hover,.ft-mobile-ticket:hover { background:var(--gold-light); box-shadow:0 8px 18px rgba(41,220,255,.25); transform:translateY(-1px); }.ft-ticket-cta i,.ft-mobile-ticket i { width:6px; height:6px; border-radius:50%; background:#03141a; box-shadow:0 0 0 3px rgba(3,20,26,.13); }
  ...
  @media(max-width:768px){.ft-header{padding-top:10px}.ft-header.is-scrolled{padding-top:7px}.ft-header-bar{min-height:58px;border-radius:15px;padding:7px 8px 7px 9px}.ft-brand-mark{width:40px;height:40px}.ft-brand-mark img{width:37px;height:37px}.ft-brand-name{display:none}.ft-desktop-nav{display:none}.ft-mobile-toggle{display:grid}.ft-mobile-drawer{top:76px}}
`
