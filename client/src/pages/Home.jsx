...
const EVENT_AUTOPLAY_MS = 5000 // 5s — enough to read the event name

function eventMeta(event) {
  const date = new Date(event.date)
  if (Number.isNaN(date.getTime())) return 'Date and time to be announced'
  return `${date.toLocaleDateString('en-PK', { day: 'numeric', month: 'short', year: 'numeric' })} Â· ${date.toLocaleTimeString('en-PK', { hour: '2-digit', minute: '2-digit' })}`
}
    ...
    <section className="mc-event-banner">
      <div className="container mc-event-shell">
        <div className="mc-event-copy">
          ...
          <Link className="mc-text-link" to="/events">Explore all events <span>Â·</span></Link>
        ...
        {loadingEvents ? <div className="mc-event-placeholder"><span className="mc-spinner" />Loading current events</div> : featured ? <div className="mc-event-carousel" role="region" aria-roledescription="carousel" aria-label="Current live events" onMouseEnter={() => setIsEventCarouselPaused(true)} onMouseLeave={() => setIsEventCarouselPaused(false)}>
          <Link key={featured.id} className="mc-feature-event" to={`/events/${featured.id}/whatsapp`} state={featured.is_review_fixture ? { reviewEvent: featured } : undefined} aria-label={`View tickets for ${featured.name}`}>
            ...
            <div className="mc-feature-content"><p>Featured this week</p><h2>{featured.name}</h2><div><span>{eventMeta(featured)}{reviewMode && ' Â· Review preview'}</span><strong>{cheapestTicket(featured)}</strong></div></div>
          </Link>
        ...
        </div> : <div className="mc-event-placeholder mc-empty-event"><p className="mc-kicker"><i /> Event desk</p><h2>New event listings will appear here.</h2><Link to="/events" className="mc-text-link">Browse events <span>Â·</span></Link></div>}
      </div>
    </section>

    <section className="mc-partner-section">
      <div className="container">
        <div className="mc-section-header"><div><p className="mc-kicker"><i /> Portfolio</p><h2>IN GOOD <em>COMPANY.</em></h2></div><Link to="/portfolio" className="mc-text-link">See the work <span>Â·</span></Link></div>
        {portfolioProjects.length ? <div className="mc-project-window"><div className="mc-project-rail" aria-label="Real projects and clients featured in our portfolio">{projectRail.map((project, index) => <Link to="/portfolio" className="mc-project-card" key={[project.id || project.client_name || project.event_name, index].join('-')}><div className="mc-project-image">{project.image_url ? <img src={project.image_url} alt={project.event_name || project.client_name || 'Portfolio project'} /> : <div className="mc-project-image-empty" />}<p>{project.is_featured ? 'Featured project' : 'Portfolio work'}</p></div><div className="mc-project-copy"><p>{project.client_name || 'Client project'}</p><h3>{project.event_name || project.client_name || 'View project'}</h3><span>View portfolio <b>â†’</b></span></div></Link>)}</div></div> : <div className="mc-partner-empty"><p className="mc-kicker"><i /> Portfolio</p><h3>Project work will appear here when the portfolio is available.</h3></div>}
      </div>
    </section>
